...
const QuickTrade = () => {
	...
	useEffect(() => {
		// fetches current price when currency changes
		const fetchPrice = async () => {
			if (!selectedCurrency) return;
			try {
				const priceData = await apiService.fetchCryptoPrice(selectedCurrency.toLowerCase());
	...
	useEffect(() => {
		// calculates fee (1%) and total when amount or price changes
		const amountNum = parseFloat(amount) || 0;
		const priceNum = parseFloat(price) || 0;
		const calculatedTotal = amountNum * priceNum;
		const calculatedFee = calculatedTotal * FEE_PERCENTAGE;
		
		setFee(calculatedFee);
		setTotal(calculatedTotal + calculatedFee);
	}, [amount, price]);
	...
	const handleBuy = async (e) => {
		...
		try {
			...
			const result = await apiService.placeOrder(signer, {
			...
			addNotification({
				type: 'success',
				title: 'Orden de Compra Colocada',
				message: `Orden de compra de ${amount} ${selectedCurrency} @ $${price} colocada exitosamente`,
			});
			...
			addNotification({
				...
				message: error.message || 'Error al colocar la orden de compra',
			});
		} finally {
			setLoading(false);
		}
	...
	const handleSell = async (e) => {
		...
		try {
			...
			addNotification({
				type: 'success',
				title: 'Orden de Venta Colocada',
				message: `Orden de venta de ${amount} ${selectedCurrency} @ $${price} colocada exitosamente`,
			});
			...
			addNotification({
				type: 'error',
				title: 'Error en la Orden',
				message: error.message || 'Error al colocar la orden de venta',
			});
		} finally {
			setLoading(false);
		}
			...
			<div className="card">
				<div className="card-header d-sm-flex d-block pb-0 border-0">
					<div>
						...
						<p className="fs-12">Compra y vende criptomonedas rápidamente</p>
					</div>
					...
					<div className="basic-form">
						<form className="form-wrapper">
							...
							<div className="form-group">
								<div className="input-group input-group-lg">
									<div className="input-group-prepend">
										<span className="input-group-text">Total USD</span>
									</div>
									<input 
										type="number" 
										className="form-control" 
							...
							<div className="row mt-4 align-items-center">
								<div className="col-lg-6">
									<div>
										<p className="fs-14">
											{isConnected 
												? 'Ingresa la cantidad y precio para realizar la operación'
												: 'Conecta tu wallet para realizar operaciones'
											}
										</p>
									</div>
								</div>
								<div className="col-lg-6">
									<div className="d-flex justify-content-end">
										<button
											onClick={handleBuy}
											disabled={!isConnected || loading || !amount || !price}
										...
										<button
											onClick={handleSell}
											disabled={!isConnected || loading || !amount || !price}
											className="btn btn-danger ms-3 rounded"
										>
											SELL
