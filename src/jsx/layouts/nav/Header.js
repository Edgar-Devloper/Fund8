...
const Header = ({ onNote }) => {
  ...
  const toggleTheme = () => {
    const next =
      background && background.value === "light"
        ...
        <nav className="navbar navbar-expand">
          <div className="collapse navbar-collapse justify-content-between">
            ...
            <ul className="navbar-nav header-right main-notification">
              {/* Theme toggle - visually matches the other circular icons */}
              <li className="nav-item">
                <button
                  type="button"
                ...
                >
                  ...
                  >
                    {/* show moon when currently light (click → dark), show sun when currently dark */}
                    {background && background.value === "light" ? (
                      <span
                        dangerouslySetInnerHTML={{
                          __html: `
<svg role="img" aria-label="Luna" width="24" height="24" viewBox="0 0 24 24"
     xmlns="http://www.w3.org/2000/svg">
  <style>
    :root {
      --moon: #D7DAE3;        /* relleno de la luna */
      --moon-dark: #A9AEBC;   /* cráteres/sombras */
      --stroke: #342E59;      /* trazo opcional */
    }
  </style>
  <defs>
    <!-- Máscara: blanco = visible, negro = recortado -->
    <mask id="crescent_toggle_1">
      <rect width="100%" height="100%" fill="#000"/>
      <circle cx="12" cy="12" r="6" fill="#fff"/>
      <circle cx="15.2" cy="9.8" r="6" fill="#000"/>
    </mask>
  </defs>

  <!-- disco recortado -->
  <g mask="url(#crescent_toggle_1)">
    <circle cx="12" cy="12" r="6" fill="var(--moon)" stroke="var(--stroke)" stroke-width="1"/>
    <!-- cráteres sutiles -->
    <circle cx="10.1" cy="11.1" r="0.65" fill="var(--moon-dark)" opacity=".8"/>
    <circle cx="12.3" cy="13.1" r="0.5"  fill="var(--moon-dark)" opacity=".7"/>
    <circle cx="13.4" cy="9.3"  r="0.4"  fill="var(--moon-dark)" opacity=".6"/>
  </g>
</svg>
                          `,
                        }}
                      ...
                      >
                        <defs>
                          <style>
                            {":root{--sun-fill:#FFD54F;--sun-stroke:#342E59;}"}
                          </style>
                        </defs>
                        <g
                          fill="none"
                        ...
                        >
                          <circle
                            ...
                            stroke="var(--sun-stroke)"
                          />
                          <path d="M12 2.2v3.2M12 18.6v3.2M2.2 12h3.2M18.6 12h3.2M4.45 4.45l2.25 2.25M17.3 17.3l2.25 2.25M4.45 19.55l2.25-2.25M17.3 6.7l2.25-2.25" />
                        </g>
                      </svg>
                    )}
                    {/* small badge like the template (empty by default) */}
                    <span
